...
연관된 객체를 처음부터 데이터베이스에서 조회하는 것이 아닌 실제 사용하는 시점에 조회할 수 있음
...
- .find() 를 통해서 영속성 컨텍스트에 Entity 가 없으면 데이터베이스를 조회

- 이렇게 Entity를 조회하면 실제 사용하지 않아도 데이터베이스를 조회하게 됨

- 조회를 미루고 싶으면 .getReference() 를 사용

- 해당 메소드를 사용하면, 데이터베이스를 조회하지 않고, 실제 Entity 객체도 생성 안함

- 대신, 데이터베이스 접근을 위임한 프록시 객체를 생성
...
- 연관 관계를 설정할 때는 식별자 값만을 사용하므로, 프록시를 사용하면 데이터베이스 접근 횟수를 줄일 수 있음