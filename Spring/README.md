...
- 개발자가 작성한 코드의 처리과정을 위임받은 존재

- Spring에서 의존성 주입(DI)를 이용하여 관리하는 컨테이너가 바로 Spring Container
...
- BeanFactory
  ...
  - 의존성 주입의 기본사항을 제공하는 가장 단순한 컨테이너

  - BeanFactory는 Bean을 생성하고 분배하는 책임을 지는 클래스

  - Bean 자체가 필요하게 되기 전까지는 인스턴스화를 하지 않음

<br>

- ApplicationContext

  - BeanFactory와 유사한 기능을 하지만 더 많은 기능 지원

  - 텍스트 메시지 관리 (인코딩)

  - 이미지같은 파일자원을 로드할 수 있는 방법 제공

  - Listener로 등록된 Bean에게 이벤트 발생을 알려줌

  - Lazy Loading이 아니라 Context 초기화 시점에 싱글톤 Bean을 미리 로드
...
- 보통 객체지형 프로그래밍에서 New, 인터페이스 호출 등으로 객체를 생성하고 소멸시킴

- IOC란 이렇게 원래 개발자의 의무인 인스턴스의 생성부터 소멸까지 객체의 생명주기를 컨테이너가 관리해주는 것을 말함

- 이것을 실제 구현한게 의존성 주입(DI)
